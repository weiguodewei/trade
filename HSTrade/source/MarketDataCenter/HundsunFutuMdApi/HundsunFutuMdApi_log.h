#if !defined(HUNDSUNFUTUMDAPI_LOG_H)
#define HUNDSUNFUTUMDAPI_LOG_H
...
///行情日志
class CHundsunFutuMdApi_log
{
public:
	CHundsunFutuMdApi_log(char* szFundAccount)
	{
		logLock.Lock();
		...
		sprintf(szFileName,"./Debug/%s/HundsunFutuMdLog_%d_%d.log", szFundAccount, now.GetDate(), now.GetTimeStamp());
		m_fp = fopen(szFileName, "ab");
		logLock.UnLock();

		bLogInfoStop = false;
		trd.Start(HundsunFutuMdLog, 0, this);
	}

	virtual ~CHundsunFutuMdApi_log()
	{
		logLock.Lock();
		fflush(m_fp);
		fclose(m_fp);
...
private:
	...
	CThread                 trd;
	CMessageQueue<CMdApiLog*> msgPush;
	ManualLock              logLock;
	FILE *					m_fp;

	static void HundsunFutuMdLog(void* lpvoid);
};
